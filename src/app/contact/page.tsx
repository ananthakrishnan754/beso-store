...
export default function ContactPage() {
  return (
    <section className="max-w-7xl mx-auto px-6 md:px-12 py-12">
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-16">
        {CONTACT_OPTIONS.map((opt) => (
          <a
            ...
            className="bg-beso-card rounded-2xl border border-white/[0.06] p-6 hover:border-white/10 transition-colors block"
          >
            <span className="text-3xl mb-3 block">{opt.icon}</span>
            <h3 className="font-semibold text-white mb-1">{opt.title}</h3>
            <p className="text-sm text-white/40 mb-4">{opt.desc}</p>
      ...
      {/* Bulk orders CTA */}
      <div className="bg-gradient-to-br from-beso-lime/10 to-beso-dark rounded-3xl border border-beso-lime/20 p-8 md:p-12 text-center mb-16">
      ...
      <div>
        ...
        <div className="space-y-4">
          {FAQ.map((item) => (
            <details
              key={item.q}
              className="bg-beso-card rounded-2xl border border-white/[0.06] group"
            >
              <summary className="px-6 py-4 cursor-pointer text-white font-medium list-none flex items-center justify-between">
                {item.q}
